...
class RabbitPikaEventConsumer(object):
    ...
    def connect(self):
        ...
        """
        LOGGER.info('Connecting to %s', config['BROKER_HOST'])
        credentials = pika.credentials.PlainCredentials(
            username=config['AMQPTASKCONSUMER_USER'], password=config['AMQPTASKCONSUMER_PASS'])
        parameters = pika.ConnectionParameters(host=config['BROKER_HOST'],
    ...
    def start_consuming(self):
        ...
        self._consumer_tag = self._channel.basic_consume(
            config['BROKER_EVENT_QUEUE'], self.on_message)
    ...
    def on_message(self, _unused_channel, basic_deliver, properties, body):
        ...
        variables = headers.get("variable_map")
        LOGGER.info(f'Received: {headers}')
        LOGGER.info(f'Received: {basic_deliver.routing_key} / {user} / {name} / {variables}')
...
class ReconnectingRabbitPikaEventConsumer(threading.Thread):
    ...
    def __init__(self):
        ...
        self._consumer = RabbitPikaEventConsumer()
